"""
Moku:Pro Physical Platform Model

This module defines the physical hardware characteristics of the Moku:Pro platform.
Based on official specifications and the moku library's conventions.

Key Features:
- 4 instrument slots
- 4 analog inputs/outputs (BNC connectors)
- 1 GHz sampling rate (highest in Moku family)
- 16-bit ADC/DAC resolution
- Professional-grade performance

References:
- Moku library: moku.instruments._mim.MultiInstrument
- Official specs: https://www.liquidinstruments.com/products/moku-pro/
"""
...
class DIOPort(BaseModel):
    ...
    The Moku:Pro has a 16-pin bidirectional DIO header accessible via ribbon cable.
    Individual pins can be configured as inputs or outputs.

    Attributes:
        num_pins: Total number of DIO pins (16 for Moku:Pro)
    ...
    sample_rate_msa: int = Field(default=1000, description="Sample rate in MSa/s (1 GHz)")
    connector_type: str = Field(default='ribbon_cable', description="Physical connector type")


class MokuProPlatform(BaseModel):
    """
    Moku:Pro Physical Platform Model.

    Represents the complete physical interface of the Moku:Pro device including:
    - 4 analog inputs/outputs (BNC connectors)
    - Digital I/O header (16 pins)
    - 4 Multi-instrument slots
    - 1 GHz sampling rate (1000 MSa/s)
    - 16-bit ADC/DAC resolution
    ...
    Attributes:
        ...
        device_name: User-assigned device name (e.g., 'MokuPro-001')
    ...
    Example:
        >>> moku = MokuProPlatform(
        ...     ip_address='192.168.1.200',
        ...     device_name='MokuPro-001'
        ... )
        >>> print(f"{moku.name}: {moku.clock_mhz} MHz clock")
        Moku:Pro: 1000 MHz clock
    """

    # Platform identification
    name: str = Field(default='Moku:Pro', description="Platform name")
    hardware_id: str = Field(default='mokupro', description="Hardware identifier (moku library)")
    ...
    # Clock
    clock_mhz: int = Field(default=1000, description="System clock frequency in MHz (1 GHz)")
...
MOKU_PRO_PLATFORM = MokuProPlatform()
